...
export function BatchCandidatesBoard({
  ...
  if (selectedBatch) {
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        {profileCandidateId && <CandidateProfileModal candidateId={profileCandidateId} onClose={() => setProfileCandidateId(null)} />}
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {batches.length === 0 && <p className="col-span-full text-center text-sm text-slate-400">No batches yet.</p>}
    </div>
  );
}
