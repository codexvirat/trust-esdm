...
/** This portal is for Workshop Managers (scoped to their assigned batches). */
export async function requireWorkshopManagerRole(): Promise<{ accessToken: string; user: SessionUser }> {
  const session = await requireSession();
  if (session.user.roleCode !== "workshop_manager") {
