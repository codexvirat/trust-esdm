...
export function DeleteWorkshopButton({ projectId, workshopId, workshopTitle }: { projectId: string; workshopId: string; workshopTitle: string }) {
    ...
    <button
      ...
      onClick={() => {
        if (window.confirm(`Delete "${workshopTitle}"? This cannot be undone.`)) {
          startTransition(() => {
            deleteWorkshopAction(projectId, workshopId);
          });
        }
      }}
