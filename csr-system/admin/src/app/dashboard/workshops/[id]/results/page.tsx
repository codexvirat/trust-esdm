...
import type { Assessment, AssessmentAttempt, Batch, WorkshopSummary } from "@/lib/types";
...
export default async function WorkshopResultsPage({
  params,
  searchParams,
}: {
  ...
  const [workshop, batches, assessments] = await Promise.all([
    apiFetch<WorkshopSummary>(`/workshops/${workshopId}?${orgQuery}`, { accessToken }),
    apiFetch<Batch[]>(`/workshops/${workshopId}/batches?${orgQuery}`, { accessToken }),
    apiFetch<Assessment[]>(`/workshops/${workshopId}/assessments?${orgQuery}`, { accessToken }),
  ]);

  const attemptsByAssessment = await Promise.all(
    assessments.map((a) => apiFetch<AssessmentAttempt[]>(`/workshops/${workshopId}/assessments/${a._id}/attempts?${orgQuery}`, { accessToken })),
  );
