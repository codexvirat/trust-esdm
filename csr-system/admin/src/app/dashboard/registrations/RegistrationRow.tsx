...
export function RegistrationRow({
  ...
  const rejectWithId = rejectRegistrationAction.bind(null, projectId, registration._id);
  const [rejectState, rejectAction, rejectPending] = useActionState(rejectWithId, initialState);

  const handleApprove = () => {
    setApproveError(null);
    startApprove(async () => {
      const result = await approveRegistrationAction(projectId, registration._id);
