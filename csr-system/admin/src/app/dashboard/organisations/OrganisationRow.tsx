...
export function OrganisationRow({ projectId, organisation }: { projectId: string; organisation: Organisation }) {
  ...
  return (
    <tr className="hover:bg-slate-50">
          ...
          <button
            ...
            onClick={() => {
              if (window.confirm(`Delete organisation "${organisation.name}"? It will no longer be selectable on the registration form.`)) {
                startTransition(() => deleteOrganisationAction(projectId, organisation._id));
              }
            }}
      ...
      {editing &&
        createPortal(
          <EditOrganisationModal projectId={projectId} organisation={organisation} onClose={() => setEditing(false)} />,
          document.body,
        )}
    </tr>
  );
