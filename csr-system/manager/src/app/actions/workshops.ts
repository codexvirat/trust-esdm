...
export async function deleteBatchAction(workshopId: string, batchId: string): Promise<string | void> {
  const { accessToken } = await requireManagerRole();
  try {
...
export async function removeWorkshopManagerAssignmentAction(workshopId: string, batchId: string, assignmentId: string): Promise<string | void> {
  const { accessToken } = await requireManagerRole();
  try {
...
export async function addDayPlanEntryAction(
  ...
  try {
    await apiFetch(`/workshops/${workshopId}/batches/${batchId}/day-plan`, {
      method: "POST",
      accessToken,
...
export async function removeDayPlanEntryAction(workshopId: string, batchId: string, entryId: string): Promise<string | void> {
  const { accessToken } = await requireManagerRole();
  try {
    await apiFetch(`/workshops/${workshopId}/batches/${batchId}/day-plan/${entryId}`, { method: "DELETE", accessToken });
  } catch (err) {
