...
export async function deleteOrganisationAction(organisationId: string): Promise<void> {
  const { accessToken } = await requireManagerRole();
  await apiFetch(`/organisations/${organisationId}`, { method: "DELETE", accessToken });
  revalidatePath("/dashboard/organisations");
}
